        ...
        if not api_key:
            ...
            if provider == "siliconflow":
                raise APIKeyMissingError(
                    "硅基流动 API Key 未设置",
                    hint="本 skill 默认使用硅基流动免费 API。\n"
                    "请先到 https://cloud.siliconflow.cn 注册账号并创建 API Key（免费额度充足）。\n"
                    "然后设置环境变量: export SILICONFLOW_API_KEY=\"sk-xxxxx\"\n"
                    "或运行: local-rag setup\n"
                    "配置完成后可执行: local-rag info 检查状态。",
                )
